...
(function () {
    ...
    <div class="shell">
      <main>
        ...
        <div class="content-grid">
          ...
          ${overlay && document.body.dataset.widget === 'full-reveal' ? `<section class="overlay-secret-reveal" aria-labelledby="overlay-secret-title"><div class="overlay-secret-image"><img id="overlay-secret-image" src="${esc(new URL('assets/images/donations/secret-placeholder.jpg', rootURL).href)}" alt=""><span id="overlay-secret-question" aria-hidden="true">?</span></div><div class="overlay-secret-copy"><h2 id="overlay-secret-title">Nākamais Florida atklājums</h2><p class="overlay-secret-amount"><strong id="overlay-secret-remaining"></strong><span id="overlay-secret-suffix">līdz atklāšanai</span></p><progress id="overlay-secret-progress" max="1" value="0" aria-label="Nākamā noslēpuma progress"></progress><div class="overlay-secret-labels"><span id="overlay-secret-collected"></span><span id="overlay-secret-target"></span></div></div></section>` : ''}
        ...
        <details class="studio-bar"><summary><span id="source-badge" class="status">Demonstrācijas dati</span><span class="controls-label">${icon('settings', 14)} Vadība</span></summary><div class="studio-actions"><button class="button" id="support-button">${icon('heart', 15)} Atbalstīt tiešraidi ${icon('arrow', 15)}</button><button class="button" id="overlay-button">${icon('screen', 15)} OBS pārklājums</button></div><p class="local-note" id="local-note"></p></details>
      </main>
    </div>
  ...
  function render() {
    ...
    if (overlay) {
      ...
      updateMissionTrack();
      const overlayReveal = $('overlay-secret-progress');
      if (overlayReveal) {
        const secret = window.DonationGoals.progress(s.goalCents, data.total);
        const complete = !secret.next;
        $('overlay-secret-title').textContent = complete ? 'Visi Florida noslēpumi atklāti!' : 'Nākamais Florida atklājums';
        $('overlay-secret-remaining').textContent = money(complete ? s.goalCents : secret.remaining);
        $('overlay-secret-suffix').textContent = complete ? 'mērķis sasniegts' : 'līdz atklāšanai';
        overlayReveal.max = secret.target || 1;
        overlayReveal.value = complete ? 1 : secret.collected;
        overlayReveal.setAttribute('aria-label', complete ? 'Visi noslēpumi atklāti' : secret.next.label + ': ' + money(secret.collected) + ' no ' + money(secret.target));
        $('overlay-secret-collected').textContent = money(complete ? s.goalCents : secret.collected);
        $('overlay-secret-target').textContent = money(complete ? s.goalCents : secret.target);
        $('overlay-secret-image').src = new URL('assets/images/donations/' + (complete ? 'florida-placeholder.jpg' : 'secret-placeholder.jpg'), rootURL).href;
        $('overlay-secret-question').hidden = complete;
      }
    } else {
